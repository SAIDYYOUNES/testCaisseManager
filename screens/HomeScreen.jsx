import { Text, View, FlatList, TextInput, Button, TouchableOpacity, StyleSheet } from "react-native"
import React, { useEffect, useState } from "react";
import axios from "axios";
import Tile from "../componnents/Tile";
import Icon from 'react-native-vector-icons/Ionicons';
import Product from "../componnents/Product";

export default function HomeScreen() {
    const [searchText, setSearchText] = useState('');
    function modifyQuantity(id, quantity) {
        setDisplayedProducts(displayedProducts.map(product => product.id === id ? { ...product, quantity: quantity } : product));
    }
    function addProdect(id) {
        if (displayedProducts.find(product => product.id === id)) {
            setDisplayedProducts(displayedProducts.filter(product => product.id !== id));
            return;
        }
        let product = {
            quantity: 1,
            ...products.find(product => product.id === id)
        }
        setDisplayedProducts([...displayedProducts, product]);
    }

    const search = () => {
        setSearchResults(products.filter(product => product.product_name.includes(searchText)));
    }
    const [products, setProducts] = useState(
        [

            {
                "id": 1,
                "product_name": "Pancakes",
                "price": 16.38
            },
            {
                "id": 2,
                "product_name": "Spaghetti Bolognese",
                "price": 8.4
            },
            {
                "id": 3,
                "product_name": "Tomato Soup",
                "price": 8.22
            },
            {
                "id": 4,
                "product_name": "Tiramisu",
                "price": 23.46
            },
            {
                "id": 5,
                "product_name": "Grilled Salmon",
                "price": 14.12
            },
            {
                "id": 6,
                "product_name": "Grilled Salmon",
                "price": 12.62
            },
            {
                "id": 7,
                "product_name": "Vegetable Stir Fry",
                "price": 11.37
            },
            {
                "id": 8,
                "product_name": "Veggie Burger",
                "price": 20.61
            },
            {
                "id": 9,
                "product_name": "Quiche Lorraine",
                "price": 26.09
            },
            {
                "id": 10,
                "product_name": "Pad Thai",
                "price": 18.77
            },
            {
                "id": 11,
                "product_name": "Lamb Chops",
                "price": 19.62
            },
            {
                "id": 12,
                "product_name": "Beef Burger",
                "price": 25.72
            },
            {
                "id": 13,
                "product_name": "Chocolate Cake",
                "price": 11.55
            },
            {
                "id": 14,
                "product_name": "Chocolate Cake",
                "price": 5.58
            },
            {
                "id": 15,
                "product_name": "Apple Pie",
                "price": 15.63
            },
            {
                "id": 16,
                "product_name": "Grilled Salmon",
                "price": 6.62
            },
            {
                "id": 17,
                "product_name": "Grilled Salmon",
                "price": 24.9
            },
            {
                "id": 18,
                "product_name": "Grilled Salmon",
                "price": 22.66
            },
            {
                "id": 19,
                "product_name": "Pad Thai",
                "price": 12.95
            },
            {
                "id": 20,
                "product_name": "Risotto",
                "price": 21.91
            },
            {
                "id": 21,
                "product_name": "Quiche Lorraine",
                "price": 21.3
            },
            {
                "id": 22,
                "product_name": "Apple Pie",
                "price": 9.65
            },
            {
                "id": 23,
                "product_name": "Tomato Soup",
                "price": 15.81
            },
            {
                "id": 24,
                "product_name": "Apple Pie",
                "price": 6.34
            },
            {
                "id": 25,
                "product_name": "Grilled Salmon",
                "price": 5.92
            },
            {
                "id": 26,
                "product_name": "Chocolate Cake",
                "price": 23.17
            },
            {
                "id": 27,
                "product_name": "Beef Burger",
                "price": 9.62
            },
            {
                "id": 28,
                "product_name": "French Onion Soup",
                "price": 29.49
            },
            {
                "id": 29,
                "product_name": "French Onion Soup",
                "price": 8.63
            },
            {
                "id": 30,
                "product_name": "Chocolate Cake",
                "price": 24.58
            },
            {
                "id": 31,
                "product_name": "Pad Thai",
                "price": 10.55
            },
            {
                "id": 32,
                "product_name": "Pulled Pork Sandwich",
                "price": 26.49
            },
            {
                "id": 33,
                "product_name": "Tomato Soup",
                "price": 8.29
            },
            {
                "id": 34,
                "product_name": "Veggie Burger",
                "price": 18.64
            },
            {
                "id": 35,
                "product_name": "Fish Tacos",
                "price": 28.78
            },
            {
                "id": 36,
                "product_name": "Fish Tacos",
                "price": 21.52
            },
            {
                "id": 37,
                "product_name": "Falafel Wrap",
                "price": 12.7
            },
            {
                "id": 38,
                "product_name": "Falafel Wrap",
                "price": 28.56
            },
            {
                "id": 39,
                "product_name": "Lamb Chops",
                "price": 22.14
            },
            {
                "id": 40,
                "product_name": "Caesar Salad",
                "price": 5.5
            },
            {
                "id": 41,
                "product_name": "Sushi Platter",
                "price": 7.95
            },
            {
                "id": 42,
                "product_name": "Pad Thai",
                "price": 17.12
            },
            {
                "id": 43,
                "product_name": "Steak Frites",
                "price": 27.03
            },
            {
                "id": 44,
                "product_name": "Roast Chicken",
                "price": 9.5
            },
            {
                "id": 45,
                "product_name": "Cheese Omelette",
                "price": 8.02
            },
            {
                "id": 46,
                "product_name": "Beef Burger",
                "price": 5.18
            },
            {
                "id": 47,
                "product_name": "Cheese Omelette",
                "price": 22.35
            },
            {
                "id": 48,
                "product_name": "Grilled Salmon",
                "price": 28.17
            },
            {
                "id": 49,
                "product_name": "Quiche Lorraine",
                "price": 29.36
            },
            {
                "id": 50,
                "product_name": "Pancakes",
                "price": 28.16
            },
            {
                "id": 51,
                "product_name": "Apple Pie",
                "price": 19.2
            },
            {
                "id": 52,
                "product_name": "Grilled Salmon",
                "price": 14.99
            },
            {
                "id": 53,
                "product_name": "Sushi Platter",
                "price": 5.18
            },
            {
                "id": 54,
                "product_name": "Grilled Salmon",
                "price": 17.66
            },
            {
                "id": 55,
                "product_name": "Tomato Soup",
                "price": 9.28
            },
            {
                "id": 56,
                "product_name": "Tiramisu",
                "price": 25.61
            },
            {
                "id": 57,
                "product_name": "Margarita Pizza",
                "price": 25.68
            },
            {
                "id": 58,
                "product_name": "Beef Burger",
                "price": 8.2
            },
            {
                "id": 59,
                "product_name": "Tomato Soup",
                "price": 11.77
            },
            {
                "id": 60,
                "product_name": "Pancakes",
                "price": 14.8
            },
            {
                "id": 61,
                "product_name": "Vegetable Stir Fry",
                "price": 13.35
            },
            {
                "id": 62,
                "product_name": "Spaghetti Bolognese",
                "price": 8.99
            },
            {
                "id": 63,
                "product_name": "Falafel Wrap",
                "price": 6.8
            },
            {
                "id": 64,
                "product_name": "Sushi Platter",
                "price": 5.92
            },
            {
                "id": 65,
                "product_name": "Tiramisu",
                "price": 29.77
            },
            {
                "id": 66,
                "product_name": "Cheese Omelette",
                "price": 21.84
            },
            {
                "id": 67,
                "product_name": "Tomato Soup",
                "price": 15.37
            },
            {
                "id": 68,
                "product_name": "Sushi Platter",
                "price": 22.26
            },
            {
                "id": 69,
                "product_name": "French Onion Soup",
                "price": 15.19
            },
            {
                "id": 70,
                "product_name": "Falafel Wrap",
                "price": 23.85
            },
            {
                "id": 71,
                "product_name": "Beef Burger",
                "price": 24.33
            },
            {
                "id": 72,
                "product_name": "Pancakes",
                "price": 15.22
            },
            {
                "id": 73,
                "product_name": "Pulled Pork Sandwich",
                "price": 5.93
            },
            {
                "id": 74,
                "product_name": "Tiramisu",
                "price": 22.94
            },
            {
                "id": 75,
                "product_name": "Steak Frites",
                "price": 14.11
            },
            {
                "id": 76,
                "product_name": "Tomato Soup",
                "price": 8.34
            },
            {
                "id": 77,
                "product_name": "Tiramisu",
                "price": 9.61
            },
            {
                "id": 78,
                "product_name": "Cheese Omelette",
                "price": 11.36
            },
            {
                "id": 79,
                "product_name": "Grilled Salmon",
                "price": 17.95
            },
            {
                "id": 80,
                "product_name": "Tiramisu",
                "price": 5.73
            },
            {
                "id": 81,
                "product_name": "French Onion Soup",
                "price": 21.0
            },
            {
                "id": 82,
                "product_name": "Cheese Omelette",
                "price": 23.47
            },
            {
                "id": 83,
                "product_name": "Pulled Pork Sandwich",
                "price": 26.44
            },
            {
                "id": 84,
                "product_name": "Sushi Platter",
                "price": 20.07
            },
            {
                "id": 85,
                "product_name": "Cheese Omelette",
                "price": 20.64
            },
            {
                "id": 86,
                "product_name": "Grilled Salmon",
                "price": 23.12
            },
            {
                "id": 87,
                "product_name": "Pancakes",
                "price": 12.28
            },
            {
                "id": 88,
                "product_name": "Margarita Pizza",
                "price": 17.13
            },
            {
                "id": 89,
                "product_name": "Veggie Burger",
                "price": 25.95
            },
            {
                "id": 90,
                "product_name": "Chocolate Cake",
                "price": 19.95
            },
            {
                "id": 91,
                "product_name": "Caesar Salad",
                "price": 23.16
            },
            {
                "id": 92,
                "product_name": "Beef Burger",
                "price": 27.68
            },
            {
                "id": 93,
                "product_name": "Sushi Platter",
                "price": 19.82
            },
            {
                "id": 94,
                "product_name": "Chocolate Cake",
                "price": 6.14
            },
            {
                "id": 95,
                "product_name": "Veggie Burger",
                "price": 29.99
            },
            {
                "id": 96,
                "product_name": "Risotto",
                "price": 29.63
            },
            {
                "id": 97,
                "product_name": "Grilled Salmon",
                "price": 28.52
            },
            {
                "id": 98,
                "product_name": "Lamb Chops",
                "price": 15.39
            },
            {
                "id": 99,
                "product_name": "Veggie Burger",
                "price": 24.77
            },
            {
                "id": 100,
                "product_name": "Apple Pie",
                "price": 10.13
            }
        ]
    );
    const [searchResults, setSearchResults] = useState(products);
    const [displayedProducts, setDisplayedProducts] = useState([]);

    async function fetch() {
        try {

            const { data } = await axios.get('http:localhost:3000/products');
            setProducts(data);
        } catch (error) {
            console.log(error.message);
        }
    }
    useEffect(() => {
        fetch();
    }, []);
    return (
        <View style={{
            backgroundColor: 'black',
            height: '100%',
            paddingBottom: 10,
            marginTop: 30

        }}>
            <View style={{
                flex: 1,
                padding: 5
            }}>
                <View style={{

                    borderWidth: 2,
                    borderColor: '#AFAFAF',
                    height: '100%',

                }}>
                    <FlatList
                        data={displayedProducts}
                        renderItem={({ item }) => <Product modifyproduct={modifyQuantity} title={item.product_name} price={item.price} quantity={item.quantity} id={item.id} />}
                        contentContainerStyle={{alignItems: 'center',paddingTop: 10}}
                    />
                </View>
            </View>
            <View style={{
                flexDirection: 'row',
                padding: 5
            }}>


                <TextInput style={{
                    flex: 5,
                    backgroundColor: 'white',
                    borderRadius: 40,
                    marginHorizontal: 10,
                    padding: 10,
                    marginBottom: 10,
                    paddingLeft: 40
                }} placeholder="Chercher un produit"
                    onChangeText={setSearchText}
                    value={searchText}
                />
                <TouchableOpacity style={{
                    position: 'absolute',
                    left: 20,
                    top: 15,
                    zIndex: 1
                }}>
                    <Icon name="search" size={20} color="black" />
                </TouchableOpacity>
                <TouchableOpacity onPress={search} style={{
                    flex: 2,
                    backgroundColor: '#CB001E',
                    borderRadius: 50,
                    padding: 10,
                    marginBottom: 10,
                    alignItems: 'center',

                }}>
                    <Text style={{ color: 'white' }}>Chercher</Text>
                </TouchableOpacity>
            </View>
            <FlatList style={{
                flex: 1,
                paddingHorizontal: 10,

            }}
                columnWrapperStyle={{
                    justifyContent: "space-between",
                    marginBottom: 10

                }}
                data={searchResults}
                numColumns={3}
                renderItem={({ item }) => <Tile addproduct={addProdect} title={item.product_name} price={item.price} id={item.id} />}
            />
        </View>
    );
}
const styles = StyleSheet.create({
    searchSection: {
        flex: 1,
        flexDirection: 'row',
        justifyContent: 'center',
        alignItems: 'center',
        backgroundColor: '#fff',
    },
    searchIcon: {
        padding: 10,
    },
    input: {
        flex: 1,
        paddingTop: 10,
        paddingRight: 10,
        paddingBottom: 10,
        paddingLeft: 0,
        backgroundColor: '#fff',
        color: '#424242',
    },
})